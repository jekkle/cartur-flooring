...
dotnet build (Join-Path $root "src\CarturFlooring.csproj") -c Release
...
Copy-Item (Join-Path $root "src\bin\Release\net472\CarturFlooring.dll") $plugins -Force

New-Item -ItemType Directory -Force -Path $dist | Out-Null
$zip = Join-Path $dist "Carturs_Flooring-$version.zip"
...
# file literally named "plugins\CarturFlooring.dll" instead of the folder.
...
$nexus = Join-Path $dist "Carturs_Flooring-$version-Nexus.zip"
if (Test-Path $nexus) { Remove-Item $nexus }
$archive = [System.IO.Compression.ZipFile]::Open($nexus, "Create")
try {
    [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
        $archive, (Join-Path $plugins "CarturFlooring.dll"), "BepInEx/plugins/CarturFlooring.dll") | Out-Null
}
finally { $archive.Dispose() }
