...
param(
    ...
    #
    # This one carries its version in the name, unlike the other five, because that is
    # how the page was first set up by hand. The match therefore has to be the exact
    # current name, while the new version is published under DisplayName below - or
    # every future release would be titled "1.0.0" whatever version it actually was.
    [string]$FileName = "Cartur's Flooring 1.0.0",

    # What the new version is called on the page. Version-less on purpose: Nexus already
    # shows the version in its own column beside it.
    [string]$DisplayName = "Cartur's Flooring"
)
...
Write-Host "published: $DisplayName $version"
